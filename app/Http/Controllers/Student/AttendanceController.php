...
class AttendanceController extends Controller
{
    ...
    function AttendanceStore(Request $request)
    {
        ...
        try {
            ...
            if (empty($check_attendance)) {
                ...
                return response()->json(['message' => 'Attendance Submited Successfully', 'status' => 200]);
            } else {
                return response()->json(['message' => 'Attendance Already Submited!', 'status' => 409]);
            }
    ...
    function TeacherShow()
    {
        try {
            ...
            return Employee::where('role', 'Teacher')->select('id','name')->get();
        } catch (\Exception $e) {
            return $e->getMessage();
        }
    }
    function AssignCourseTeacher(Request $request)
    {
        try {
            ...
            foreach ($course_id as $key => $id) {
                ...
                $course->assigned_by_id = $request->assign_id;
                $course->save();
            };
            return response()->json(['message' => 'Course Teacher Assigned Successfully'], 201);
        } catch (\Exception $e) {
